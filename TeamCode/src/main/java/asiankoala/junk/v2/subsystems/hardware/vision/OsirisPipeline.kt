...
abstract class OsirisPipeline : OpenCvPipeline() {
    ...
    override fun processFrame(input: Mat): Mat {
        ...
        val matLeft = workingMatrix.submat(
            leftRowStart, leftRowStart + leftHeight,
            leftColStart, leftColStart + leftWidth
        )
        val matCenter = workingMatrix.submat(
            centerRowStart, centerRowStart + centerHeight,
            centerColStart, centerColStart + centerWidth
        )
        val matRight = workingMatrix.submat(
            rightRowStart, rightRowStart + rightHeight,
