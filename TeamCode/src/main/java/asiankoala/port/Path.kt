...
//        }
        ...
        val powers = Pose(finalXPower, finalYPower, finalTurnPower)

        Logger.logInfo("pose", pose)
        Logger.logInfo("powers", powers.rawString())
        Logger.logInfo("rel angle", realRelativeAngle.degrees)
        Logger.logInfo("absolute angle", absolutePointAngle)
        Logger.logInfo("move lookahead", movementLookahead)
        Logger.logInfo("clipped dist", clippedDistanceToEnd)
        Logger.logInfo("clipped point", clippedToPath)
        Logger.logInfo("follow index", currFollowIndex)

        return Pair(powers, absolutePointAngle)
    }
