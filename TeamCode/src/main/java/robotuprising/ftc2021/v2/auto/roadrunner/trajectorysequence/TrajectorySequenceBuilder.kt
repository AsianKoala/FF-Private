package robotuprising.ftc2021.v2.auto.roadrunner.trajectorysequence
...
import robotuprising.ftc2021.v2.auto.roadrunner.trajectorysequence.sequencesegment.SequenceSegment
import robotuprising.ftc2021.v2.auto.roadrunner.trajectorysequence.sequencesegment.TrajectorySegment
import robotuprising.ftc2021.v2.auto.roadrunner.trajectorysequence.sequencesegment.TurnSegment
import robotuprising.ftc2021.v2.auto.roadrunner.trajectorysequence.sequencesegment.WaitSegment
...
class TrajectorySequenceBuilder(
    ...
    fun addTemporalMarker(time: TimeProducer?, callback: MarkerCallback?): TrajectorySequenceBuilder {
    ...
    fun addDisplacementMarker(scale: Double, offset: Double, callback: MarkerCallback?): TrajectorySequenceBuilder {
        return addDisplacementMarker({ displacement: Double -> scale * displacement + offset }, callback)
    }

    fun addDisplacementMarker(displacement: DisplacementProducer?, callback: MarkerCallback?): TrajectorySequenceBuilder {
        displacementMarkers.add(DisplacementMarker(displacement!!, callback!!))
        return this
    }

    @JvmOverloads
    fun turn(angle: Double, maxAngVel: Double = currentTurnConstraintMaxAngVel, maxAngAccel: Double = currentTurnConstraintMaxAngAccel): TrajectorySequenceBuilder {
        pushPath()
    ...
    private fun newPath() {
        ...
        currentTrajectoryBuilder = TrajectoryBuilder(lastPose, tangent, currentVelConstraint, currentAccelConstraint, resolution)
    }
    ...
    private fun projectGlobalMarkersToLocalSegments(markers: List<TrajectoryMarker>, sequenceSegments: MutableList<SequenceSegment>): List<SequenceSegment?> {
        ...
        for ((time, callback) in markers) {
            ...
            when (segment) {
                ...
                is TurnSegment -> {
                    ...
                    newMarkers.add(TrajectoryMarker(segmentOffsetTime, callback))
                    val thisSegment = segment
                    newSegment = TurnSegment(thisSegment.startPose, thisSegment.totalRotation, thisSegment.motionProfile, newMarkers)
                }
    ...
    private fun pointToTime(sequenceSegments: List<SequenceSegment?>, point: Vector2d): Double {
        class ComparingPoints(val distanceToPoint: Double, val totalDisplacement: Double, val thisPathDisplacement: Double)
