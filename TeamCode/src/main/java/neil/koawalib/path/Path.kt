package neil.koawalib.path

import neil.koawalib.command.commands.PathCommand
import neil.koawalib.math.MathUtil
import neil.koawalib.math.MathUtil.degrees
import neil.koawalib.math.MathUtil.radians
import neil.koawalib.math.MathUtil.wrap
...
class Path(private val waypoints: List<Waypoint>, private val followAngle: Double = 90.0.radians) {
    ...
    fun update(pose: Pose): Pose {
        ...
        var movementLookahead = PurePursuitController.calcLookahead(waypoints, pose, waypoints[currFollowIndex].followDistance)


        val last = PurePursuitController.extendLine(
                waypoints[waypoints.size - 2].point, waypoints[waypoints.size - 1].point, waypoints[waypoints.size - 1].turnLookaheadDistance * 1.5)
        ...
        val turnLookahead = PurePursuitController.calcLookahead(extendedPath, pose, waypoints[currFollowIndex].turnLookaheadDistance)
        ...
        val result = PurePursuitController.pointTo(pose.heading, currFollowAngle, waypoints[currFollowIndex].maxTurnSpeed, 45.0.radians)
